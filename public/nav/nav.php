...
// Safety check if language file wasn't loaded
if (!isset($textes)) { $textes = []; }
// Safety check for $langue variable for the toggle colors
    ...
    <div class="nav-links">
        ...
        <?php if ($estConnecté): ?>
            <a href="/public/compte/monCompte.php">
                <?= $textes['nav_account'] ?? 'Compte' ?> (<?= htmlspecialchars($nom_utilisateur) ?>)
            </a>
        ...
        <span style="margin-left:15px; font-size:0.9em; border-left:1px solid #444; padding-left:15px; font-weight:bold;">
            <a href="?lang=fr" style="text-decoration:none; color:<?= $langue === 'fr' ? '#4da6ff' : '#888' ?>;">FR</a>
            <span style="color:#444; margin:0 5px;">|</span>
            <a href="?lang=en" style="text-decoration:none; color:<?= $langue === 'en' ? '#4da6ff' : '#888' ?>;">EN</a>
        </span>
    </div>
</nav>